...
1. Add this repository as `labs-src/cloth-simulation` using an HTTPS Git submodule.
...
5. Run `npm ci` in `web` and `VITE_BASE=/labs/cloth-simulation/ npm run build`.
6. Copy `web/dist/*` into the website output at `labs/cloth-simulation/`.
